...
exports.ingestKnowledgeFlow = void 0;
exports.handleIngestKnowledge = handleIngestKnowledge;
...
exports.ingestKnowledgeFlow = genkit_js_1.ai.defineFlow({
    name: 'ingestKnowledge',
    inputSchema: genkit_1.z.void(),
    outputSchema: genkit_1.z.object({ count: genkit_1.z.number(), status: genkit_1.z.string() }),
}, async () => {
    // Optional: Clear existing chunks to avoid duplicates during dev
    // In production, you'd manage this more carefully
    const chunksRef = config_js_1.db.collection('rag_chunks');
...
async function handleIngestKnowledge(req, res) {
    try {
        const result = await (0, exports.ingestKnowledgeFlow)();
        res.json(result);
    }
    catch (error) {
...
//# sourceMappingURL=ingestKnowledge.js.map